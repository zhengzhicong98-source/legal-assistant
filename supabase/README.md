...
**RAG 流程：**
1. 接收用户问题，调用智谱 embedding-3 生成 2000 维查询向量
2. 在 `legal_knowledge` 表中执行 pgvector 余弦相似度检索（`match_legal_docs_hybrid` 混合检索）
...
将知识库文档文本转换为 2000 维向量存储到 pgvector。
...
| `category` | text | 分类 |
| `content` | text | 正文内容 |
| `embedding` | vector(2000) | 文本向量（智谱 embedding-3） |
| `created_at` | timestamptz | 创建时间 |
...
3. `00003_change_embedding_vector_1536_to_1024.sql` — 将向量维度从 1536 改为 1024（适配智谱 embedding-3 早期配置）
